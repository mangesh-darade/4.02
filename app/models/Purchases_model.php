...
class Purchases_model extends CI_Model
{
    ...
    public function addPurchase($data, $items)
    {
		...
		$this->db->insert('purchase_items', $item);
                ...
                $_taxPurchaseID =  $purchase_id;
                $taxAtrr = $this->sma->taxAtrrClassification($item['tax_rate_id'],$item['net_unit_cost'],$item['unit_quantity'],$purchase_item_id,$_taxPurchaseID,'p');
                 
                 /*Add New field to purchase_items,purchases cgst,igst,sgst 17-1-2020*/
                $tax_ItemAtrr = $this->sma->taxArr_rate($item['tax_rate_id'],$item['net_unit_cost'],$item['unit_quantity'],$purchase_item_id,$_taxPurchaseID,'p');
                ...
                $this->db->update('purchase_items', array('gst_rate' => $tax_ItemAtrr[0]['attr_per'], 'cgst' => $cgst,'sgst' => $sgst, 'igst' => $igst), array('id' => $purchase_item_id));
             ...
             $this->db->update('purchases', array('cgst' => $total_cgst, 'sgst' => $total_sgst,'igst' => $total_igst), array('id' =>  $purchase_id));
    ...
    public function updatePurchase($id, $data, $items = array())
    {
        ...
        if ($this->db->update('purchases', $data, array('id' => $id)) && $this->db->delete('purchase_items', array('purchase_id' => $id))) {
            ...
            foreach ($items as $item) {
             ...
             $this->db->update('purchases', array('cgst' => $total_cgst, 'sgst' => $total_sgst,'igst' => $total_igst), array('id' =>  $id));
