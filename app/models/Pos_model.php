...
class Pos_model extends CI_Model {
    ...
    public function addSale($data = array(), $items = array(), $payments = array(), $sid = NULL) {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            // Invoice No
            $invoice_no_array = ['invoice_no' => $this->sma->invoice_format($sale_id, date()),];
            ...
            foreach ($items as $item) {
                ...
                $tax_ItemAtrr = $this->sma->taxArr_rate($item['tax_rate_id'], $item['net_unit_price'], $item['quantity'], $sale_item_id, $sale_id);
            ...
            $this->db->update('sales', array('cgst' => $total_cgst, 'sgst' => $total_sgst, 'igst' => $total_igst), array('id' => $sale_id));
    ...
    public function getcalpartial() {
        $sdate = date('Y-m-d 00:00:00');
        $edate = date('Y-m-d 23:59:59');
        $q = $this->db->select('SUM( COALESCE( grand_total, 0 ) ) - SUM( COALESCE( paid, 0 ) ) AS partial_due', FALSE)
    ...
    public function getTodayDueSales() {
        $sdate = date('Y-m-d 00:00:00');
        $edate = date('Y-m-d 23:59:59');
        $q = $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total', FALSE)
                        ->where(['payment_status' => 'due'])->get_where('sales', "date >= '$sdate'");
    ...
    public function getTodaySalesPaid() {
        $sdate = date('Y-m-d 00:00:00');
        $edate = date('Y-m-d 23:59:59');
    ...
    public function getTodayDCSales() {
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_dc_slips, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >', $date)->where('paid_by', 'DC');
    ...
    public function getTodayGiftCardSales() {
        $date = date('Y-m-d 00:00:00');
    ...
    public function getTodayDepSales() {
        $date = date('Y-m-d 00:00:00');
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_gc_slips, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
    ...
    public function getTodayPaymentOptionSales($payOpt = '') {
        if (empty($payOpt))
            return false;
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_cheques, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
                ->join('sales', 'sales.id=payments.sale_id', 'left')
                ->where('type', 'received')->where('payments.date >', $date)->where('paid_by', $payOpt);
