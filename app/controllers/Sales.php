...
class Sales extends MY_Controller {
    ...
    public function getSales($warehouse_id = null) {
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                    ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no,id as invoice_no, biller, customer, sale_status, (grand_total+rounding), paid, (grand_total+rounding-paid) as balance, payment_status, attachment, return_id")
                    ->from('sales');
    ...
    public function all_sale_lists_filter($pdf = NULL, $xls = NULL) {
        ...
        if ($pdf || $xls) {
            ...
            $si .= " GROUP BY {$this->db->dbprefix('sale_items')}.sale_id ) FSI";
            $this->datatables
                    ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, id as invoice_no, biller, customer, sale_status, (grand_total+rounding), paid, (grand_total+rounding-paid) as balance, payment_status, attachment, return_id, if(pos=1, 'POS', if(offline_sale=1, 'Offline', if(eshop_sale=1, 'Eshop', if(up_sales=1, 'up_sales', 'Sale')))) as sale_type")
                    ->from('sales')->join($si, 'FSI.sale_id=sales.id', 'left');
            if ($TypeOfModeSale) {
                if ($TypeOfModeSale == 'Sale') {
                    $this->datatables->where('sales.eshop_sale =', 0);
    ...
    public function add($quote_id = null) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_unit_quantity)) {
                    ...
                    $total += $this->sma->formatDecimal(($item_net_price * $item_unit_quantity), 4); //17/05/19
                }
            }
            if (empty($products)) {
    ...
    <ul class="dropdown-menu pull-right" role="menu">
        ...
        $this->datatables
                ->select("deliveries.id as id, date, do_reference_no, sale_reference_no, customer, address, status, delivery_type ,  attachment ")
                ->from('deliveries');
        if ($this->session->userdata('view_right') == '0') {
...
//        $pdf_link = anchor('sales/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                    ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, invoice_no as reference_no, sale_invoice_no as invoice_no, biller, customer, sale_status, (grand_total+rounding), paid, (grand_total+rounding-paid) as balance, payment_status, attachment, return_id")
                    ->from('orders');
