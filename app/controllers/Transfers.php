           ...
           if($last_status !='completed'){
            ...
            for ($r = 0; $r < $i; $r++) {
               ...
               $item_net_cost = $this->sma->formatDecimal($_POST['net_cost'][$r]);
				...
				if($status!='completed'){
					if($last_status !='sent'){            
						if( ($item_request_quantity - $sent_quantity) < $_POST['quantity'][$r]){
               ...
               /*  else { */
                    ...
                    if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
						...
						//echo 'main bal '.$balance_qty.' item '.$item_quantity.' order '.$ordered_quantity.' qty_balance '.$quantity_balance; exit;
...
//exit;
        if ($this->form_validation->run() == true && $this->transfers_model->addTransfer($data, $products)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('transfers'), 'page' => lang('transfers')), array('link' => '#', 'page' => lang('Add Request')));
            $meta = array('page_title' => lang('transfer_quantity'), 'bc' => $bc);
