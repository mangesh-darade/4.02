	...
	$purchase_id= $this->input->get('purchase_id') ? $this->input->get('purchase_id') : NULL;
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
				...
				$item_batch_number = isset($_POST['batch_number'][$r]) ? $_POST['batch_number'][$r] : null;
	...
	if($this->input->post('previous_url')){
		...
		redirect('reports/purchases_gst_report');
            ...
            foreach ($inv_items as $item) {
                ...
                $units = $this->site->getUnitsByBUID($row->base_unit);
                $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                $ri = $c; //$this->Settings->item_addition ? $row->id : $c;
    ...
    public function suggestions()
    {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $pr[] = array('id' => ($c + $r), 'item_id' => $row->id, 'image'=> $product->image, 'label' => $row->name . " (" . $row->code . ")", 
                    'row' => $row, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
                $r++;
            }
