...
class Pos extends MY_Controller {
    ...
    <ul class="dropdown-menu pull-right" role="menu">
        ...
        if ($warehouse_id) {
            ...
            /* $this->datatables
              ...
              ->group_by('sales.id'); */
            $this->datatables
                    ->select($this->db->dbprefix('sales') . ".id as id, date, reference_no," . $this->db->dbprefix('sales') . ".id as invoice_id, biller, customer, (grand_total+rounding), paid, (grand_total+rounding-paid) as balance, sale_status, payment_status, delivery_status, companies.email as cemail")
                    ->from('sales')
                    ->join('companies', 'companies.id=sales.customer_id', 'left')
                    ->where('pos', 1)
                    ->group_by('sales.id');
    ...
    public function index($sid = null) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_quantity = $this->sma->formatDecimal($_POST['product_base_quantity'][$r], 4);
                ...
                $item_unit_quantity = $this->sma->formatDecimal($_POST['quantity'][$r], 4); ///($_POST['product_base_quantity'][$r]=="undefined")?$_POST['quantity'][$r]:$_POST['product_base_quantity'][$r];
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($item_unit_price) && isset($item_unit_quantity)) {
                    ...
                    $product_tax += $this->sma->formatDecimal(($unit_tax * $item_unit_quantity), 6);
                ...
                if ($suspended_sale = $this->pos_model->getOpenBillByID($sid)) {
                    ...
                    foreach ($inv_items as $item) {
                        ...
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",
                            'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'note' => $row->note);
                        $c++;
                    }
    ...
    public function getProductDataByCode($code = null, $warehouse_id = null) {
        ...
        if ($row) {
            ...
            /*
             * ************************************
             * Comment for do not check purchase item balance quantity for overselling on 
             * ************************************
             */
            /* $row->quantity = 0;
    ...
    public function today_sale() {
        ...
        $this->data['gcsales'] = $this->pos_model->getTodayGiftCardSales();
        $this->data['othersales'] = $this->pos_model->getTodayOtherSales();
        ...
        /* 5-11-2019 */
        $this->data['depositsales'] = $this->pos_model->getTodayDepSales($register_open_time);
        /* end */
        /* ---- 13-03-19 --- */
        $this->data['duepayment'] = $this->pos_model->getTodayDueSales();
        $this->data['duepartial'] = $this->pos_model->getcalpartial(); //20-03-19
        /* ----End  13-03-19 --- */
        $this->data['totalsales'] = $this->pos_model->getTodaySales();
        $this->data['totalsalespaid'] = $this->pos_model->getTodaySalesPaid();
...
//$this->data['customer'] = $this->pos_model->getCompanyByID($this->pos_settings->default_customer);
        ...
        if (!empty($products)) {
            ...
            foreach ($products as $product) {
                ...
                if (file_exists('assets/uploads/thumbs/' . $product->image)) {
                    if ($product->image != "") {
                        $imgsrc = 'assets/uploads/thumbs/' . $product->image;
                    } else {
    ...
    public function split_order_save($sid = null) {
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            if ($suspend) {
                ...
                if ($result = $this->pos_model->suspendSale($data, $products, $did)) {
    ...
    /* Show Deposit balance Show 6-11-2019 */
    ...
    public function get_dependancy() {
        ...
        $get_customer = $this->db->select('id,name,phone')->where($getKyes['0'], $get_data)->get('sma_companies')->row();
    ...
    public function addcustomer() {
        $response = array();
        $field = array('name' => $this->input->get('name'), 'phone' => $this->input->get('mobile_no'), 'group_id' => 3, 'customer_group_id' => 1, 'group_name' => 'customer');
...
//print_r( $res );exit;
    ...
    public function dailysale_email() {
        ...
        if ($to != '')
            $this->sma->send_email($to, $subject, $msg);
    }
