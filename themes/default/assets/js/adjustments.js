...
function getVariant_Detail(VarientId, ProductId, ItemId){
       
        var WarehouseId =  $('#qawarehouse').val();
        qaitems[ItemId].row.option = VarientId;
        localStorage.setItem('qaitems', JSON.stringify(qaitems));
        var base_path = window.location.pathname;
        var geturl_path = base_path.split("/");
        var url_pass = window.location.origin+'/'+geturl_path[1]+'/get_variant_details';  
        $.ajax({
               type: 'get',
               url:url_pass,
                data:{
               ...
               success:function(result){
                   ...
                   $.each(Res, function(key,value){
                       $('#ShowQty_'+ItemId).text(formatDecimal(value.quantity));
                       var qty = formatDecimal(value.quantity)
                       qaitems[ItemId].row.product_qty = qty;
                       localStorage.setItem('qaitems', JSON.stringify(qaitems));
    ...
    if (localStorage.getItem('qaitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var type = item.row.type ? item.row.type : '';
             var quantity =item.row.product_qty;
            var opt = $('<select id="poption" name="variant[]" class="form-control select " onchange="return getVariant_Detail(this.value, '+product_id+', '+ item_id +');" />'); //rvariant onchange="return getVariantDetails(this.value, '+item_id+');"
            ...
            tr_html += '<td class="text-right" id="ShowQty_'+ item_id +'"><input name="shockqty"  type="hidden"  id="shockqty_' + item_id +'"  value="'+formatDecimal(quantity)+'">'+formatDecimal(quantity)+'</td>';
